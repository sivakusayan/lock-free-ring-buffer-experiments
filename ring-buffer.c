...
size_t ring_buffer_size(size_t count, size_t item_size) {
    ...
    // Even if the buffer size didn't overflow, make sure adding the header size to it won't either!
...
int ring_buffer_init(void* ring_buffer, size_t count, size_t item_size) {
    ...
    struct ring_buffer_header header = ((struct ring_buffer*)ring_buffer)->header;
    header.count = count;
